...
class LeadRepositoryTest {
    ...
    void setUp() {
        ...
        leadTwo = new Lead("lead two", 641564612L, "leadtwo@test.com", "Company test 2", new Date(), new Date(0L), "user one", "user one", userOne);
        leadThree = new Lead("lead three", 641564613L, "leadthree@test.com", "Company test 3", new Date(), new Date(0L), "user two", "user one", userTwo);
        leadFour = new Lead("lead four", 641564614L, "leadfour@test.com", "Company test 4", new Date(), new Date(0L), "user two", "user one", userTwo);
        leadFive = new Lead("lead five", 641564615L, "leadfive@test.com", "Company test 5", new Date(), new Date(0L), "user two", "user one", userTwo);
        leadSix = new Lead("lead six", 641564616L, "leadsix@test.com", "Company test 6", new Date(), new Date(0L), "user two", "user one", userTwo);
    ...
    void findById_ReturnAccount_OpportunityFound() {
        ...
        assertEquals(641564611L, optionalLead.get().getPhoneNumber());

        optionalLead = leadRepository.findById(leadTwo.getId());
        assertTrue(optionalLead.isPresent());
        assertEquals(641564612L, optionalLead.get().getPhoneNumber());
    }
